...
Usage (launched automatically by main.py):
    python overlay.py <transcript_file> <control_file> [opacity] [width] [height] [font_size]
"""
...
if __name__ == "__main__":
    if len(sys.argv) < 3:
        print("Usage: python overlay.py <transcript_file> <control_file> [opacity] [width] [height] [font]")
        sys.exit(1)

    transcript_path = sys.argv[1]
    control_path = sys.argv[2]
    opacity = float(sys.argv[3]) if len(sys.argv) > 3 else 0.82
    width = int(sys.argv[4]) if len(sys.argv) > 4 else 620
    height = int(sys.argv[5]) if len(sys.argv) > 5 else 230
    font_size = int(sys.argv[6]) if len(sys.argv) > 6 else 13
